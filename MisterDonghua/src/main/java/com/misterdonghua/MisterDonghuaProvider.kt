...
class MisterDonghuaProvider : MainAPI() {
    override var mainUrl = "https://misterdonghua.com"
    override var name = "Mister Donghua"
    ...
    private fun Element.toSearchResult(): SearchResponse? {
        ...
        val title = linkEl.attr("title").ifBlank {
            this.selectFirst("div.tt h2")?.text() ?: this.selectFirst("div.tt")?.text()
        }?.trim() ?: return null

        val rawPoster = this.selectFirst("img")?.attr("data-src")?.ifBlank { null }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val rawPoster = posterElement?.attr("data-src")?.ifBlank { null }
